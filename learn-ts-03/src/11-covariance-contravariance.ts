...
interface Producer2<out T> {
  // when returning the type parameter
  produce: () => T;
}
...
// prepareSnacks(cookies, cookieQualityCheck); //! Nope
// prepareSnacks(snacks, cookieQualityCheck); //! Nope
prepareCookies(cookies, snackQualityCheck); //✔️ OK

//? What if we turn `strictFunctionTypes` on and off?

//* What do variance helpers do for you? -- it speeds up type checking

interface Example<in T> {
  package: (item: T) => void;
  // produce: () => T;  //! does not work
}

interface Example2<out T> {
  // package: (item: T) => void; //! does not work
  produce: () => T;
}
export default {};
